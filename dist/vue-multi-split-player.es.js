var Et = (a, e) => (e = Symbol[a]) ? e : Symbol.for("Symbol." + a);
var v = (a, e, i) => new Promise((t, l) => {
  var n = (d) => {
    try {
      u(i.next(d));
    } catch (c) {
      l(c);
    }
  }, o = (d) => {
    try {
      u(i.throw(d));
    } catch (c) {
      l(c);
    }
  }, u = (d) => d.done ? t(d.value) : Promise.resolve(d.value).then(n, o);
  u((i = i.apply(a, e)).next());
}), L = function(a, e) {
  this[0] = a, this[1] = e;
}, W = (a, e, i) => {
  var t = (o, u, d, c) => {
    try {
      var g = i[o](u), C = (u = g.value) instanceof L, E = g.done;
      Promise.resolve(C ? u[0] : u).then((f) => C ? t(o === "return" ? o : "next", u[1] ? { done: f.done, value: f.value } : f, d, c) : d({ value: f, done: E })).catch((f) => t("throw", f, d, c));
    } catch (f) {
      c(f);
    }
  }, l = (o) => n[o] = (u) => new Promise((d, c) => t(o, u, d, c)), n = {};
  return i = i.apply(a, e), n[Et("asyncIterator")] = () => n, l("next"), l("throw"), l("return"), n;
};
import { ref as p, computed as Ct, onMounted as ft, onBeforeUnmount as vt, watch as de, nextTick as It } from "vue";
import { Notification as Je } from "element-ui";
function wt(a) {
  var e = /^1[3,5,8,2,4,6,7,9]\d{9}$/;
  return !!e.test(a);
}
function Bt() {
  let a = navigator.userAgent;
  return ["Android", "iPhone", "SymbianOS", "Windows Phone", "iPad", "iPod", "Mobile"].filter((i) => a.indexOf(i) !== -1).length;
}
function st() {
  return /Chrome/.test(navigator.userAgent) && /Google Inc/.test(navigator.vendor);
}
function Qt(a) {
  let e = new RegExp("[?&]" + a + "=([^&#]*)").exec(window.location.href);
  return e ? e[1] : "";
}
function Dt(a) {
  var e = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/;
  return e.test(a);
}
function Mt(a) {
  var e = /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/;
  return e.test(a);
}
Date.prototype.format = function(a) {
  var e = {
    "M+": this.getMonth() + 1,
    //月份
    "d+": this.getDate(),
    //日
    "h+": this.getHours(),
    //小时
    "m+": this.getMinutes(),
    //分
    "s+": this.getSeconds(),
    //秒
    "q+": Math.floor((this.getMonth() + 3) / 3),
    //季度
    S: this.getMilliseconds()
    //毫秒
  };
  /(y+)/.test(a) && (a = a.replace(RegExp.$1, (this.getFullYear() + "").substr(4 - RegExp.$1.length)));
  for (var i in e) new RegExp("(" + i + ")").test(a) && (a = a.replace(RegExp.$1, RegExp.$1.length == 1 ? e[i] : ("00" + e[i]).substr(("" + e[i]).length)));
  return a;
};
function kt(a, e) {
  return new Date(a).format(e);
}
function bt(a = 0, e = 0) {
  let i = 86400, t = 60 * 60, l = 60, n = 0;
  if (i = parseInt(a / i), t = parseInt(a / t % 24), l = parseInt(a / l % 60), n = parseInt(a % 60), t < 10 && (t = "0" + t), l < 10 && (l = "0" + l), n < 10 && (n = "0" + n), e === 0)
    return i === 0 ? i = "" : i = `${i}-`, t === "00" ? t = "" : t = `${t}:`, `${i}${t}${l || "00"}:${n || "00"}`;
  if (e === 1)
    return `${t || "00"}:${l || "00"}:${n || "00"}`;
  if (e === 2)
    return `${i || "00"}-${t || "00"}:${l || "00"}`;
}
function ot(a, e = 500) {
  let i = Array.prototype.slice.call(arguments, 2);
  a.timer || (a.timer = setTimeout(() => {
    a.apply(this, i), a.timer = "";
  }, e));
}
function J(a, e = 500) {
  return (...i) => {
    clearTimeout(a.timer), a.timer = setTimeout(() => {
      a.apply(this, i);
    }, e);
  };
}
function Rt(a, e = 1e3) {
  let i = Array.prototype.slice.call(arguments, 2);
  a.looked || (a.apply(this, i), a.looked = !0), !a.timer && (a.timer = setTimeout(() => {
    a.looked = !1, a.timer = "";
  }, e));
}
function ye(a, e = 500) {
  let i = Array.prototype.slice.call(arguments, 2);
  clearTimeout(a.timer), a.timer = setTimeout(() => {
    a.apply(this, i);
  }, e);
}
function St(a, e = 1e3) {
  let i = Array.prototype.slice.call(arguments, 2);
  a.looked || (a.apply(this, i), a.looked = !0), clearTimeout(a.timer), a.timer = setTimeout(() => {
    a.looked = !1;
  }, e);
}
function ct(a) {
  return a.endsWith(".m3u8");
}
function Ee(a) {
  switch (Object.prototype.toString.call(a)) {
    case "[object Object]":
      return "object";
    case "[object Array]":
      return "array";
    case "[object String]":
      return "string";
    case "[object Number]":
      return "number";
    case "[object Boolean]":
      return "boolean";
    case "[object Symbol]":
      return "symbol";
    default:
      return typeof a;
  }
}
function xt(a) {
  let e = Array.prototype.slice.call(arguments, 1);
  if (a.clicked) {
    a.call(this, ...e), a.clicked = !1;
    return;
  }
  a.clicked = !0, setTimeout(() => {
    a.clicked = !1;
  }, 500);
}
function ut(a) {
  let e = Object.freeze({
    "-1": "老师",
    0: "老师",
    1: "老师全景",
    2: "学生",
    3: "学生全景",
    4: "板书",
    5: "双视图",
    6: "跟踪",
    7: "老师左辅助",
    8: "老师右辅助",
    50: "板书辅助"
  }), i = a.split(",");
  return new Set(i.map((t) => e[t]));
}
const dt = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  dateResult: kt,
  dblclick: xt,
  debounce: ot,
  debounceBind: J,
  debounceExecute: Rt,
  filterTime: bt,
  filterVoiceConfigId: ut,
  getParam: Qt,
  isChrome: st,
  isEmail: Dt,
  isM3u8: ct,
  isMobile: Bt,
  isPhone: wt,
  isValidIP: Mt,
  luck: St,
  throttle: ye,
  typeDetection: Ee
}, Symbol.toStringTag, { value: "Module" }));
function Ce(a, e, i, t, l, n, o, u) {
  var d = typeof a == "function" ? a.options : a;
  return e && (d.render = e, d.staticRenderFns = i, d._compiled = !0), n && (d._scopeId = "data-v-" + n), {
    exports: a,
    options: d
  };
}
const Ot = {
  model: {
    prop: "current",
    event: "change"
  },
  props: {
    // 播放进度
    current: {
      type: Number,
      default: 0
    },
    // 播放时长
    duration: {
      type: Number,
      default: 0
    },
    // 当前的最后分屏路径URL
    httpPath: {
      type: String,
      default: ""
    },
    // mark点列表
    mark: {
      type: Array,
      default: (a) => []
    }
  },
  data() {
    return {
      common: dt,
      disabled: !1,
      startX: 0,
      startPosition: 0,
      distance: 0,
      // 可滑行的距离
      is_show_pic: !0,
      // 是否展示缩略图
      // 缩略图信息
      thumbnail: {
        url: "",
        // 图片地址
        time: ""
        // 时间
      },
      show_block: !1,
      // 是否展示移动块
      loading: !1,
      note_item: {},
      // 当前查看的知识点
      is_draging: !1
      // 是否拖动过程中
    };
  },
  computed: {
    lineStyle() {
      if (this.distance && this.current && this.duration)
        return {
          width: this.current * (this.distance / this.duration) + "px"
        };
    },
    slideStyle() {
      if (this.distance && this.current && this.duration)
        return {
          left: this.current * (this.distance / this.duration) + "px"
        };
    }
  },
  mounted() {
    this._getDistance();
  },
  methods: {
    /**
     * @description 点击line修改时间
     */
    handleClick(a) {
      if (this.is_draging || !a.target.id)
        return;
      let e = a.target.getBoundingClientRect().left, i = (a.pageX - e) * (this.duration / this.distance);
      this.updateCurrentTime(i);
    },
    /**
     * @description 鼠标/触摸板开始
     */
    onButtonDown(a) {
      this.disabled || (a.preventDefault(), this.onDragStart(a), window.addEventListener("mousemove", this.onDragging), window.addEventListener("touchmove", this.onDragging), window.addEventListener("mouseup", this.onDragEnd), window.addEventListener("touchend", this.onDragEnd), window.addEventListener("contextmenu", this.onDragEnd));
    },
    /**
     * @description 拖拽开始
     */
    onDragStart(a) {
      this.is_draging = !0, a.type === "touchstart" && (a.pageY = a.touches[0].pageY, a.pageX = a.touches[0].pageX), this.startPosition = parseFloat(this.current), this.startX = a.pageX;
    },
    /**
     * @description 拖拽过程中
     */
    onDragging(a) {
      if (!this.is_draging)
        return;
      a.type === "touchmove" && (a.pageY = a.touches[0].pageY, a.pageX = a.touches[0].pageX);
      let e = 0;
      e = (a.pageX - this.startX) * (this.duration / this.distance);
      let i = this.startPosition + e;
      this.updateCurrentTime(i);
    },
    /**
     * @description 拖拽结束
     */
    onDragEnd() {
      this.is_draging && (window.removeEventListener("mousemove", this.onDragging), window.removeEventListener("touchmove", this.onDragging), window.removeEventListener("mouseup", this.onDragEnd), window.removeEventListener("touchend", this.onDragEnd), window.removeEventListener("contextmenu", this.onDragEnd), setTimeout((a) => {
        this.is_draging = !1;
      }, 50));
    },
    updateCurrentTime(a) {
      a < 0 && (a = 0), a >= this.duration && (a = this.duration), window.requestIdleCallback ? window.requestIdleCallback((e) => this.$emit("update", a)) : ot((e) => this.$emit("update", a), 100);
    },
    /**
     * @description 鼠标移入，显示移动块
     */
    handleMouseEnter(a) {
    },
    /**
     * @description 鼠标移动，修改移动块
     */
    onDraggingThumbnail(a) {
      if (!this.show_block)
        return;
      let e = document.getElementById("videoPlayer"), i = document.getElementById("indicator"), t = document.getElementById("thumbnail"), l = a.clientX - this._getLeft(e);
      i.style.transform = `translateX(${l}px)`, t.style.transform = `translateX(${l - 180 / 2 + 4}px)`;
      let n = l * (this.duration / this.distance);
      this.$set(this.thumbnail, "time", n.toFixed(0)), this.mark.length && this._getNoteItem(n), !this.loading && (this.loading = !0);
    },
    /**
     * @description 鼠标移出，隐藏移动块
     */
    handleMouseLeave() {
      this.show_block = !1;
    },
    /**
     * @description 获取当前所在区域的知识点
     * @param {Number} time: 当前所在时间点
     */
    _getNoteItem(a) {
      let e = this.mark.findIndex((i) => a < i.startSecond);
      this.mark[e - 1] ? this.note_item = this.mark[e - 1] : this.note_item = {};
    },
    /**
     * @description 点击知识点
     * @param {Object} item: 当前选中项
     * @param {Number} index: 选中项下标index
     */
    handle_click_note(a, e) {
      this.$emit("update", a.startSecond);
    },
    // 获取知识点的位置信息
    getNotePosition(a) {
      if (this.distance && this.duration)
        return a * (this.distance / this.duration);
    },
    /**
     * @description 获取元素距离页面左边的间距
     * @param {Object} el: 要获取的元素
     */
    _getLeft(a) {
      let e = a.offsetLeft;
      return a.offsetParent !== null && (e += this._getLeft(a.offsetParent)), e;
    },
    /**
     * @description 获取滑行区域距离
     */
    _getDistance() {
      this.$nextTick((a) => {
        this.distance = document.getElementById("track").offsetWidth;
      });
    }
  }
};
var Lt = function() {
  var e = this, i = e._self._c;
  return i("div", { staticClass: "slide", attrs: { id: "videoPlayer" }, on: { mousedown: function(t) {
    return t.preventDefault(), e.handleClick(t);
  }, touchstart: function(t) {
    return t.preventDefault(), e.handleClick(t);
  }, mouseenter: e.handleMouseEnter, mousemove: e.onDraggingThumbnail, mouseleave: e.handleMouseLeave } }, [i("div", { staticClass: "custom_cursor", style: e.slideStyle, attrs: { id: "box", tabindex: "0" }, on: { mousedown: e.onButtonDown, touchstart: e.onButtonDown } }), i("div", { style: e.lineStyle, attrs: { id: "track_line" } }), i("div", { attrs: { id: "track" } }), e.show_block ? i("div", { attrs: { id: "indicator" } }, [i("i", { staticClass: "el-icon-caret-bottom" }), i("i", { staticClass: "el-icon-caret-top" })]) : e._e(), e.show_block ? i("div", { staticClass: "thumbnail", attrs: { id: "thumbnail" } }, [i("div", { staticClass: "img_wrap" }, [i("img", { attrs: { src: e.thumbnail.url, alt: "" } })]), i("div", { staticClass: "text" }, [e._v(e._s(e.note_item.content))]), i("div", { staticClass: "time" }, [e._v(e._s(e.common.filterTime(+e.thumbnail.time * 1e3, 1)))])]) : e._e(), e._l(e.mark, function(t, l) {
    return i("div", { staticClass: "noteItem", style: { left: e.getNotePosition(t.startSecond) + "px" }, on: { click: function(n) {
      return e.handle_click_note(t, l);
    } } }, [i("div", { staticClass: "note_detail" }, [i("div", { staticClass: "note_wrap" }, [i("div", { staticClass: "img_wrap" }, [i("img", { attrs: { src: t.imgUrl, alt: "" } })]), i("div", { staticClass: "note_content" }, [e._v(e._s(t.content))]), e._m(0, !0)]), i("div", { staticClass: "note_time _flex_center" }, [e._v(e._s(e.common.filterTime(t.startSecond, 1)))])])]);
  })], 2);
}, Gt = [function() {
  var a = this, e = a._self._c;
  return e("div", { staticClass: "icon _flex_center" }, [e("i", { staticClass: "el-icon-caret-bottom" })]);
}], zt = /* @__PURE__ */ Ce(
  Ot,
  Lt,
  Gt,
  !1,
  null,
  "7df9647d"
);
const Kt = zt.exports, Ht = {
  model: {
    prop: "value",
    event: "input"
  },
  props: {
    // 全屏状态
    value: {
      type: Boolean,
      default: !1
    },
    // 全屏的类型 0 logo全屏 1 自定义slot全屏
    type: {
      type: Number,
      default: 0
    }
  },
  date() {
    return {
      tip: ""
      // 提示信息
    };
  },
  methods: {
    // 全屏
    handleFullscreen(a) {
      let e = a || document.body;
      this.value ? document.exitFullscreen ? document.exitFullscreen() : document.mozCancelFullScreen ? document.mozCancelFullScreen() : document.webkitCancelFullScreen ? document.webkitCancelFullScreen() : document.msExitFullscreen && document.msExitFullscreen() : e.requestFullscreen ? e.requestFullscreen() : e.mozRequestFullScreen ? e.mozRequestFullScreen() : e.webkitRequestFullScreen ? e.webkitRequestFullScreen() : e.msRequestFullscreen && e.msRequestFullscreen();
    },
    // change
    handleChange(a) {
      this.handleFullscreen(a);
    }
  },
  mounted() {
    let a = document.fullscreenElement || document.mozFullScreenElement || document.webkitFullscreenElement || document.fullScreen || document.mozFullScreen || document.webkitIsFullScreen;
    a = !!a, document.addEventListener("fullscreenchange", () => {
      this.$emit("input", !this.value), this.$emit("on-change", !this.value);
    }), document.addEventListener("mozfullscreenchange", () => {
      this.$emit("input", !this.value), this.$emit("on-change", !this.value);
    }), document.addEventListener("webkitfullscreenchange", () => {
      this.$emit("input", !this.value), this.$emit("on-change", !this.value);
    }), document.addEventListener("msfullscreenchange", () => {
      this.$emit("input", !this.value), this.$emit("on-change", !this.value);
    }), this.$emit("input", a);
  }
};
var qt = function() {
  var e = this, i = e._self._c;
  return i("div", [e.type ? e._t("default", function() {
    return [e._v("点击全屏")];
  }) : i("div", { staticClass: "logo _flex_center" })], 2);
}, Pt = [], Ft = /* @__PURE__ */ Ce(
  Ht,
  qt,
  Pt,
  !1,
  null,
  "0a15f553"
);
const Tt = Ft.exports;
class Nt {
  constructor(e) {
    this.playId = e.id, this.content = (e.content || "vue-multi-split-player").toString(), this.timer = null, this.waterMarkerDom = null, this.observer_c = null, this.observer_p = null, this.create();
  }
  create() {
    return v(this, null, function* () {
      this.getElement() && this.getElement().remove(), yield this.wait(500 + Math.random() * 200);
      let e = this.content, i = document.getElementById(this.playId);
      if (!i)
        return;
      this.width = i.offsetWidth, this.height = i.offsetHeight;
      let t = document.createElement("div");
      t.style.display = "inline-block", t.style.position = "absolute", t.id = `water_marker_${this.playId}`, t.style.top = "40px", t.style.left = "40px", t.style.lineHeight = "20px", t.style.color = "#fff", t.style.fontSize = "14px", t.style.textAlign = "left", t.style.pointerEvents = "none", t.style.filter = "opacity(0.3)", t.style.textShadow = `1px 1px 0px rgba(0, 0, 0, 0.2),
      1px 2px 0px rgba(0, 0, 0, 0.2),
      1px 3px 0px rgba(0, 0, 0, 0.2)`, t.style.mixBlendMode = "lighten", t.style.zIndex = 99, t.innerHTML = e, this.waterMarkerDom = t, i == null || i.appendChild(t), this.create_animate1(t), yield this.wait(1e3), this.observe_c(t), this.observe_p(i);
    });
  }
  create_animate1(e) {
    let i = 2, t = this.width - e.offsetWidth - 40, l = this.height - e.offsetHeight - 40, n = (Math.random() * t).toFixed(0), o = (Math.random() * l).toFixed(0), u = {
      transform: `translate(calc(${n}px),calc(${o}px))`
    };
    e.animate(u, {
      duration: 0,
      fill: "forwards",
      easing: "cubic-bezier(.31,.01,1,1.27)",
      delay: 0
    }), this.timer = setInterval(() => {
      i--, i === 0 && (i = 3, clearInterval(this.timer), this.create_animate1(e));
    }, 1e3);
  }
  create_animate(e) {
    e.animate([
      {
        transform: "translate(0 ,0) rotateZ(-45deg)"
      },
      // 起始位置
      {
        transform: `translate(calc(${this.width - 40}px - 100%), 0) rotateZ(45deg)`
      },
      // 右侧
      {
        transform: `translate(calc(${this.width - 40}px - 100%), calc(${this.height - 140}px - 100%)) rotateZ(45deg)`
      },
      // 底部
      {
        transform: `translate(0, calc(${this.height - 140}px - 100%)) rotateZ(-45deg)`
      },
      // 左侧
      {
        transform: "translate(0, 0) rotateZ(-45deg)"
      }
      // 返回起始位置
    ], {
      duration: 6e3,
      iterations: 1 / 0,
      // 无限循环
      easing: "linear",
      delay: 0
    });
  }
  observe_c(e) {
    let i = {
      attributes: !0,
      characterData: !0,
      childList: !0,
      subtree: !0
    };
    this.observer_c = new MutationObserver((t, l) => {
      for (let n of t)
        switch (n.type) {
          case "attributes":
          case "childList":
          case "characterData":
            Je({
              title: "提示",
              message: "请勿篡改水印哦，尊重版权！",
              type: "warning",
              showClose: !0
            }), this.destroy(this.getElement()), this.create();
            break;
        }
    }), this.observer_c.observe(e, i);
  }
  observe_p(e) {
    let i = {
      attributes: !0,
      characterData: !0,
      childList: !0,
      subtree: !0
    };
    this.observer_p = new MutationObserver((t, l) => {
      for (let n of t)
        switch (n.type) {
          case "childList":
            let o = Array.from(n.removedNodes);
            o.length && o[0].id === `water_marker_${this.playId}` && (Je({
              title: "提示",
              message: "请勿删除水印哦，尊重版权！",
              type: "warning",
              showClose: !0
            }), this.destroy(this.getElement()), this.create());
            break;
        }
    }), this.observer_p.observe(e, i);
  }
  destroy(e = this.getElement()) {
    var i, t;
    clearInterval(this.timer), (i = this.observer_c) == null || i.disconnect(), (t = this.observer_p) == null || t.disconnect(), e && e.remove();
  }
  getElement() {
    return this.waterMarkerDom || document.getElementById(`water_marker_${this.playId}`);
  }
  wait(e) {
    return v(this, null, function* () {
      return new Promise((i) => setTimeout(i, e));
    });
  }
}
const te = "data:image/gif;base64,";
var Ye, Ve, Ze, Ue, w;
function X(a, e, i, t, l) {
  var n = {};
  return Object.keys(t).forEach(function(o) {
    n[o] = t[o];
  }), n.enumerable = !!n.enumerable, n.configurable = !!n.configurable, ("value" in n || n.initializer) && (n.writable = !0), n = i.slice().reverse().reduce(function(o, u) {
    return u(a, e, o) || o;
  }, n), l && n.initializer !== void 0 && (n.value = n.initializer ? n.initializer.call(l) : void 0, n.initializer = void 0), n.initializer === void 0 ? (Object.defineProperty(a, e, n), null) : n;
}
let _ = () => function(a, e, i) {
  let t = i.value;
  i.value = function(...l) {
    return t.apply(this, l);
  };
}, jt = (Ye = _(), Ve = _(), Ze = _(), Ue = _(), w = class {
  constructor(e) {
    this.config = e, this.player = {}, this.onLoadFinish = null, this.onPlayTime = null, this.onPlayState = null, this.onPlayFinish = null, this.run();
  }
  run() {
    let e = this._init(), {
      value: i,
      done: t
    } = e.next();
    t || (e.next(), e.next());
  }
  // 初始化
  _init() {
    return W(this, null, function* () {
      let {
        id: e,
        type: i,
        url: t,
        index: l,
        count: n
      } = this.config;
      if (!e)
        throw new Error("Container id is required");
      if (!t)
        throw new Error("player url is required");
      if (yield yield new L(this.createVideo(e, t)), !this.player)
        return {
          value: void 0,
          done: !0
        };
      yield yield new L(this.initEvent(this.player)), yield yield new L(this.addVttSubtitle(this.player, l, n));
    });
  }
  // 创建video
  createVideo(e, i) {
    try {
      let t = document.querySelector(e);
      if (!t)
        throw new Error("Player id is required");
      t.innerHTML = "";
      let l = document.createElement("video");
      if (l.poster = te, l.controls = !1, l.autoplay = !1, l.muted = !1, l.preload = "auto", l.volume = 0, l.classList = ["video__"], ct(i)) {
        let n = new Hls();
        n.loadSource(i), n.attachMedia(l);
      } else
        l.src = i;
      t.appendChild(l), this.player = l;
    } catch (t) {
      throw new Error("create video error" + t);
    }
  }
  // 添加VTT字幕
  addVttSubtitle(e, i, t) {
    if (window.subtitleUrl && i === t - 1) {
      let l = document.createElement("track");
      l.src = window.subtitleUrl || "", l.label = "Chinese", l.srclang = "zh", l.kind = "subtitles", l.default = " ", e.appendChild(l);
    }
  }
  // 监听video一系列事件
  initEvent(e) {
    return v(this, null, function* () {
      if (!e)
        return;
      yield new Promise((t) => setTimeout(t, 10)), ["canplay", "play", "pause", "seeked", "timeupdate", "volumechange", "ended", "error"].forEach((t) => {
        let l = t, n = `on${l.charAt(0).toUpperCase() + l.slice(1)}`;
        e.addEventListener(t, this[n].bind(this), {
          once: l === "canplay"
        });
      });
    });
  }
  onCanplay() {
    this.onLoadFinish && this.onLoadFinish();
  }
  onPlay() {
    this.onPlayState && this.onPlayState(!0);
  }
  onPause() {
    this.onPlayState && this.onPlayState(!1);
  }
  onSeeked() {
  }
  onTimeupdate() {
    this.onPlayTime && this.onPlayTime(this.player.currentTime);
  }
  onVolumechange() {
  }
  onEnded() {
    this.onPlayFinish && this.onPlayFinish();
  }
  onError() {
  }
  do() {
    this.player.play(), this.onPlayState && this.onPlayState(!0);
  }
  play() {
    this.player.play(), this.onPlayState && this.onPlayState(!0);
  }
  pause() {
    this.player.pause(), this.onPlayState && this.onPlayState(!1);
  }
  isPlaying() {
    return !this.player.paused;
  }
  seek(e) {
    requestAnimationFrame(() => {
      this.player.currentTime = e;
    });
  }
  setVoice(e) {
    this.player.volume = e;
  }
  mediaInfo() {
    return {
      duration: this.player.duration * 1e3
    };
  }
  release() {
    var e;
    try {
      this.player.pause(), this.player.src = "", this.player.load(), (e = this.player) == null || e.parentNode.removeChild(this.player);
    } catch (i) {
    }
  }
  setPlaybackRate(e) {
    this.player.playbackRate = e;
  }
}, X(w.prototype, "_init", [Ye], Object.getOwnPropertyDescriptor(w.prototype, "_init"), w.prototype), X(w.prototype, "createVideo", [Ve], Object.getOwnPropertyDescriptor(w.prototype, "createVideo"), w.prototype), X(w.prototype, "addVttSubtitle", [Ze], Object.getOwnPropertyDescriptor(w.prototype, "addVttSubtitle"), w.prototype), X(w.prototype, "initEvent", [Ue], Object.getOwnPropertyDescriptor(w.prototype, "initEvent"), w.prototype), w);
function Jt(a) {
  let e = p(null), i = p(null), t = 0.2, l = (c, g) => {
    var C, E, f, Q;
    c.value = c, i.value = g, c.value && ((C = c.value) == null || C.addEventListener("click", n), (E = c.value) == null || E.addEventListener("dblclick", n), (f = c.value) == null || f.addEventListener("mousedown", J(u, t * 1e3)), (Q = c.value) == null || Q.addEventListener("mouseup", J(d, t * 1e3)));
  }, n = (c) => {
    c.stopPropagation();
  }, o = () => {
    var c, g, C, E;
    (c = e.value) == null || c.removeEventListener("click", n), (g = e.value) == null || g.removeEventListener("dblclick", n), (C = e.value) == null || C.removeEventListener("mousedown", J(u, t * 1e3)), (E = e.value) == null || E.removeEventListener("mouseup", J(d, t * 1e3));
  }, u = (c) => {
    c.stopPropagation();
    let g = c.target;
    g.getAttribute("data-type") && a.emit("startControl", {
      id: i.value,
      type: g.getAttribute("data-type")
    });
  }, d = (c) => {
    c.stopPropagation();
    let g = c.target;
    g.getAttribute("data-type") && a.emit("stopControl", {
      id: i.value,
      type: g.getAttribute("data-type")
    });
  };
  return {
    startEventMonitoring: l,
    stopEventMonitoring: o
  };
}
var We, Xe, _e, $e, B;
function $(a, e, i, t, l) {
  var n = {};
  return Object.keys(t).forEach(function(o) {
    n[o] = t[o];
  }), n.enumerable = !!n.enumerable, n.configurable = !!n.configurable, ("value" in n || n.initializer) && (n.writable = !0), n = i.slice().reverse().reduce(function(o, u) {
    return u(a, e, o) || o;
  }, n), l && n.initializer !== void 0 && (n.value = n.initializer ? n.initializer.call(l) : void 0, n.initializer = void 0), n.initializer === void 0 ? (Object.defineProperty(a, e, n), null) : n;
}
let ee = () => function(a, e, i) {
  let t = i.value;
  i.value = function(...l) {
    return t.apply(this, l);
  };
}, Yt = (We = ee(), Xe = ee(), _e = ee(), $e = ee(), B = class {
  constructor(e) {
    this.config = e, this.disableControlList = ["PPT", "PGM", "老师全景", "学生全景"], this.player = {}, this.onLoadFinish = null, this.onPlayTime = null, this.onPlayState = null, this.onPlayFinish = null, this.replyCount = 5, this.use_control_listener = Jt(this.config.eventBus), this.run();
  }
  run() {
    let e = this._init(), {
      value: i,
      done: t
    } = e.next();
    t || (e.next(), e.next(), e.next());
  }
  // 初始化
  _init() {
    return W(this, null, function* () {
      let {
        id: e,
        classroomId: i,
        type: t,
        url: l,
        index: n,
        count: o,
        name: u
      } = this.config;
      if (!e)
        throw new Error("Container id is required");
      if (!l)
        throw new Error("player url is required");
      if (yield yield new L(this.createVideo(e, l, u)), yield yield new L(this.createControlLayout(e, i, u)), !this.player)
        return {
          value: void 0,
          done: !0
        };
      yield yield new L(this.initEvent(this.player));
    });
  }
  // 创建video
  createVideo(n, o, u) {
    return v(this, arguments, function* (e, i, t, l = 0) {
      let d = Array.from(arguments);
      try {
        let c = document.querySelector(e);
        if (!c)
          throw new Error("Player id is required");
        c.innerHTML = "";
        let g = document.createElement("video");
        g.id = `video_${e}`, g.classList = ["video__"], g.controls = !1, g.autoplay = !0, g.playsInline = !0, g.preload = "auto", g.poster = te, g.volume = 0, g.controls = !1, c.appendChild(g), this.player = yield Streamedian.player(g.id, {
          socket: i,
          // redirectNativeMediaErrors: true,
          bufferDuration: 10,
          dataHandler: this._handleData,
          errorHandler: (C) => this._handlerError(C, ...d, l)
        });
      } catch (c) {
        throw new Error(c);
      }
    });
  }
  // 创建遥控面板
  createControlLayout(e, i, t) {
    return v(this, null, function* () {
      if (!this.config.isEnableControl)
        return;
      let l = document.getElementById(e);
      (function() {
        let n = l.querySelector(".remote_control");
        n && n.parentNode.removeChild(n);
      }).apply(), !this.disableControlList.includes(t) && function() {
        let n = document.createElement("div");
        n.classList = ["remote_control"], n.innerHTML = ` <div class="activeElement" data-type="3"><i class="iconfont icon-shangxiazuoyouTriangle12"></i></div>
              <div class="activeElement" data-type="2"><i class="iconfont icon-shangxiazuoyouTriangle12"></i></div>
              <div class="activeElement" data-type="4"><i class="iconfont icon-shangxiazuoyouTriangle12"></i></div>
              <div class="activeElement" data-type="1"><i class="iconfont icon-shangxiazuoyouTriangle12"></i></div>
              <div class="action activeElement" data-type="5"><i class="el-icon-plus"></i>放大</div>
              <div class="action activeElement" data-type="6"><i class="el-icon-minus"></i>缩小</div>
              <div class="action activeElement" data-type="10"><i class="el-icon-refresh-left"></i>还原</div>`, l.appendChild(n), this.use_control_listener.startEventMonitoring(n, i);
      }.apply(this);
    });
  }
  // 监听video一系列事件
  initEvent(e) {
    ["canplay", "play", "pause", "seeked", "loadedmetadata", "timeupdate", "volumechange", "ended", "error"].forEach((t) => {
      let l = t, n = `on${l.charAt(0).toUpperCase() + l.slice(1)}`;
      e.player.addEventListener(t, this[n].bind(this), {
        once: l === "canplay"
      });
    });
  }
  onCanplay() {
    this.onLoadFinish && this.onLoadFinish();
  }
  onPlay() {
    this.onPlayState && this.onPlayState(!0);
  }
  onPause() {
    this.onPlayState && this.onPlayState(!1);
  }
  onSeeked() {
    return v(this, null, function* () {
    });
  }
  onLoadedmetadata() {
    return v(this, null, function* () {
    });
  }
  onTimeupdate() {
    var e;
    this.onPlayTime && this.onPlayTime((e = this.player) == null ? void 0 : e.player.currentTime);
  }
  onVolumechange() {
  }
  onEnded() {
    this.onPlayFinish && this.onPlayFinish();
  }
  onError() {
  }
  play() {
    var e;
    this.player.start(), (e = this.player.player) == null || e.play(), this.onPlayState && this.onPlayState(!0);
  }
  pause() {
    var e;
    this.player.stop(), (e = this.player.player) == null || e.pause(), this.onPlayState && this.onPlayState(!1);
  }
  isPlaying() {
    return this.player.isPlaying();
  }
  setVoice(e) {
    this.player.player.volume = e;
  }
  release() {
    var e, i;
    try {
      (e = this.player) == null || e.destroy(), (i = this.player.player) == null || i.parentNode.removeChild(this.player.player), this.use_control_listener.stopEventMonitoring();
    } catch (t) {
    }
  }
  // 录屏时
  _handleData(e, i) {
    let t = document.createElement(""), l = new Blob([e], {
      type: "application/mp4"
    });
    t.href = window.URL.createObjectURL(l), t.download = `${i}_${/* @__PURE__ */ new Date()}.mp4`, t.click();
  }
  /**
   * @description: 错误处理
   * @param {Object} error
   * @param {Number} retryCount
   */
  _handlerError(e, i, t, l) {
    return v(this, null, function* () {
      if (l < this.replyCount)
        yield this.release(), yield new Promise((n) => setTimeout(n, 1e3)), yield this.createVideo(i, t, l + 1);
      else
        throw new Error(`Max retry count (${this.replyCount}) reached`);
    });
  }
}, $(B.prototype, "_init", [We], Object.getOwnPropertyDescriptor(B.prototype, "_init"), B.prototype), $(B.prototype, "createVideo", [Xe], Object.getOwnPropertyDescriptor(B.prototype, "createVideo"), B.prototype), $(B.prototype, "createControlLayout", [_e], Object.getOwnPropertyDescriptor(B.prototype, "createControlLayout"), B.prototype), $(B.prototype, "initEvent", [$e], Object.getOwnPropertyDescriptor(B.prototype, "initEvent"), B.prototype), B);
var et, tt, it, b;
function he(a, e, i, t, l) {
  var n = {};
  return Object.keys(t).forEach(function(o) {
    n[o] = t[o];
  }), n.enumerable = !!n.enumerable, n.configurable = !!n.configurable, ("value" in n || n.initializer) && (n.writable = !0), n = i.slice().reverse().reduce(function(o, u) {
    return u(a, e, o) || o;
  }, n), l && n.initializer !== void 0 && (n.value = n.initializer ? n.initializer.call(l) : void 0, n.initializer = void 0), n.initializer === void 0 ? (Object.defineProperty(a, e, n), null) : n;
}
let pe = () => function(a, e, i) {
  let t = i.value;
  i.value = function(...l) {
    return t.apply(this, l);
  };
}, Vt = (et = pe(), tt = pe(), it = pe(), b = class {
  constructor(e) {
    this.config = e, this.player = {}, this.onLoadFinish = null, this.onPlayTime = null, this.onPlayState = null, this.onPlayFinish = null, this.run();
  }
  run() {
    let e = this._init(), {
      value: i,
      done: t
    } = e.next();
    t || (e.next(), e.next());
  }
  // 初始化
  _init() {
    return W(this, null, function* () {
      let {
        id: e,
        classroomId: i,
        type: t,
        url: l,
        index: n,
        count: o,
        name: u
      } = this.config;
      if (!e)
        throw new Error("Container id is required");
      if (!l)
        throw new Error("player url is required");
      if (yield yield new L(this.createVideo(e, l, u)), !this.player)
        return {
          value: void 0,
          done: !0
        };
      yield yield new L(this.initEvent(this.player));
    });
  }
  // 创建video
  createVideo(n, o, u) {
    return v(this, arguments, function* (e, i, t, l = 0) {
      Array.from(arguments);
      try {
        let d = document.querySelector(e);
        if (!d)
          throw new Error("Player id is required");
        d.innerHTML = "";
        let c = document.createElement("video");
        c.id = `video_${e}`, c.classList = ["video__"], c.controls = !1, c.autoplay = !1, c.playsInline = !0, c.preload = "auto", c.poster = te, c.volume = 0, d.appendChild(c), this.player = new ZLMRTCClient.Endpoint({
          element: c,
          // video 标签
          debug: !1,
          // 是否打印日志
          zlmsdpUrl: "http://10.0.129.131:8080/index/api/webrtc?app=live&stream=100_0&type=play",
          //流地址
          simulcast: !1,
          useCamera: !0,
          audioEnable: !0,
          videoEnable: !0,
          recvOnly: !0,
          resolution: {
            w: 1280,
            h: 720
          },
          usedatachannel: !1,
          audioId: "",
          videoId: ""
          // videoId:vdevid, // 不填选择默认的
          // audioId:adevid, // 不填选择默认的
        }), this.player.video = c;
      } catch (d) {
        throw new Error(d);
      }
    });
  }
  // 监听video一系列事件
  initEvent(e) {
    ["canplay", "play", "pause", "seeked", "loadedmetadata", "timeupdate", "volumechange", "ended", "error"].forEach((t) => {
      let l = t, n = `on${l.charAt(0).toUpperCase() + l.slice(1)}`;
      e.video.addEventListener(t, this[n].bind(this), {
        once: l === "canplay"
      });
    }), e.on(ZLMRTCClient.Events.WEBRTC_ICE_CANDIDATE_ERROR, function(t) {
      console.log("ICE 协商出错");
    }), e.on(ZLMRTCClient.Events.WEBRTC_ON_REMOTE_STREAMS, (t) => {
      this.onCanplay(), console.log("播放成功", t.streams);
    }), e.on(ZLMRTCClient.Events.WEBRTC_OFFER_ANWSER_EXCHANGE_FAILED, function(t) {
      console.log("offer anwser 交换失败", t), stop();
    }), e.on(ZLMRTCClient.Events.WEBRTC_ON_LOCAL_STREAM, function(t) {
      document.getElementById("selfVideo").srcObject = t, document.getElementById("selfVideo").muted = !0;
    }), e.on(ZLMRTCClient.Events.CAPTURE_STREAM_FAILED, function(t) {
      console.log("获取本地流失败");
    }), e.on(ZLMRTCClient.Events.WEBRTC_ON_CONNECTION_STATE_CHANGE, function(t) {
      console.log("当前状态==>", t);
    }), e.on(ZLMRTCClient.Events.WEBRTC_ON_DATA_CHANNEL_OPEN, function(t) {
      console.log("rtc datachannel 打开 :", t);
    }), e.on(ZLMRTCClient.Events.WEBRTC_ON_DATA_CHANNEL_MSG, function(t) {
      console.log("rtc datachannel 消息 :", t.data), document.getElementById("msgrecv").value = t.data;
    }), e.on(ZLMRTCClient.Events.WEBRTC_ON_DATA_CHANNEL_ERR, function(t) {
      console.log("rtc datachannel 错误 :", t);
    }), e.on(ZLMRTCClient.Events.WEBRTC_ON_DATA_CHANNEL_CLOSE, function(t) {
      console.log("rtc datachannel 关闭 :", t);
    });
  }
  onCanplay() {
    console.log("canplay"), this.onLoadFinish && this.onLoadFinish();
  }
  onPlay() {
    this.onPlayState && this.onPlayState(!0);
  }
  onPause() {
    this.onPlayState && this.onPlayState(!1);
  }
  onSeeked() {
    return v(this, null, function* () {
    });
  }
  onLoadedmetadata() {
    return v(this, null, function* () {
    });
  }
  onTimeupdate() {
    var e;
    this.onPlayTime && this.onPlayTime((e = this.player) == null ? void 0 : e.video.currentTime);
  }
  onVolumechange() {
  }
  onEnded() {
    this.onPlayFinish && this.onPlayFinish();
  }
  onError() {
  }
  play() {
    var e;
    this.player.start(), (e = this.player.video) == null || e.play(), this.onPlayState && this.onPlayState(!0);
  }
  pause() {
    var e;
    (e = this.player.video) == null || e.pause(), this.onPlayState && this.onPlayState(!1);
  }
  isPlaying() {
    return !this.player.video.paused;
  }
  setVoice(e) {
    this.player.video.volume = e;
  }
  release() {
    var e;
    try {
      this.player.close(), this.player.closeDataChannel(), this.player.video.load(), (e = this.player.video) == null || e.parentNode.removeChild(this.player.video);
    } catch (i) {
    }
  }
}, he(b.prototype, "_init", [et], Object.getOwnPropertyDescriptor(b.prototype, "_init"), b.prototype), he(b.prototype, "createVideo", [tt], Object.getOwnPropertyDescriptor(b.prototype, "createVideo"), b.prototype), he(b.prototype, "initEvent", [it], Object.getOwnPropertyDescriptor(b.prototype, "initEvent"), b.prototype), b);
var at, nt, lt, R;
function ge(a, e, i, t, l) {
  var n = {};
  return Object.keys(t).forEach(function(o) {
    n[o] = t[o];
  }), n.enumerable = !!n.enumerable, n.configurable = !!n.configurable, ("value" in n || n.initializer) && (n.writable = !0), n = i.slice().reverse().reduce(function(o, u) {
    return u(a, e, o) || o;
  }, n), l && n.initializer !== void 0 && (n.value = n.initializer ? n.initializer.call(l) : void 0, n.initializer = void 0), n.initializer === void 0 ? (Object.defineProperty(a, e, n), null) : n;
}
let me = () => function(a, e, i) {
  let t = i.value;
  i.value = function(...l) {
    return t.apply(this, l);
  };
}, Zt = (at = me(), nt = me(), lt = me(), R = class {
  constructor(e) {
    this.config = e, this.player = {}, this.onLoadFinish = null, this.onPlayTime = null, this.onPlayState = null, this.onPlayFinish = null, this.run();
  }
  run() {
    let e = this._init(), {
      value: i,
      done: t
    } = e.next();
    t || (e.next(), e.next());
  }
  // 初始化
  *_init() {
    let {
      id: e,
      type: i,
      url: t,
      index: l,
      count: n
    } = this.config;
    if (!e)
      throw new Error("Container id is required");
    if (!t)
      throw new Error("player url is required");
    if (yield this.createVideo(e, t), !this.player)
      return {
        value: void 0,
        done: !0
      };
    yield this.initEvent(this.player);
  }
  // 创建video
  createVideo(e, i) {
    try {
      let t = document.querySelector(e);
      if (!t)
        throw new Error("Player id is required");
      t.innerHTML = "";
      let l = document.createElement("video");
      t.appendChild(l), this.player = videojs(l, {
        liveui: !0,
        autoplay: st() ? "muted" : !0,
        controls: !1,
        volume: 0,
        sources: [{
          src: i,
          type: "application/x-mpegURL"
        }],
        userActions: {
          click: !1,
          // 用户单机行为
          doubleClick: !1,
          // 用户双击行为
          hotkeys: !1
          // 热键行为
        }
      });
    } catch (t) {
      throw new Error("create video error" + t);
    }
  }
  // 监听video一系列事件
  initEvent(e) {
    return v(this, null, function* () {
      if (!e)
        return;
      ["canplay", "play", "pause", "timeupdate", "volumechange", "ended", "error"].forEach((t) => {
        let l = t, n = `on${l.charAt(0).toUpperCase() + l.slice(1)}`;
        e.on(t, this[n].bind(this));
      });
    });
  }
  onCanplay() {
    this.onLoadFinish && this.onLoadFinish();
  }
  onPlay() {
    console.log("play"), this.onPlayState && this.onPlayState(!0);
  }
  onPause() {
    this.onPlayState && this.onPlayState(!1);
  }
  onTimeupdate() {
    this.onPlayTime && this.onPlayTime(this.player.currentTime());
  }
  onVolumechange() {
  }
  onEnded() {
    this.onPlayFinish && this.onPlayFinish();
  }
  onError(e) {
    console.log(e);
  }
  play() {
    this.player.play(), this.onPlayState && this.onPlayState(!0);
  }
  pause() {
    this.player.pause(), this.onPlayState && this.onPlayState(!1);
  }
  isPlaying() {
    return !this.player.paused;
  }
  seek(e) {
    requestAnimationFrame(() => {
      this.player.currentTime = e;
    });
  }
  setVoice(e) {
    this.player.volume(e);
  }
  mediaInfo() {
    return {
      duration: this.player.duration * 1e3
    };
  }
  release() {
    try {
      this.player.dispose();
    } catch (e) {
    }
  }
  setPlaybackRate(e) {
    this.player.playbackRate = e;
  }
}, ge(R.prototype, "_init", [at], Object.getOwnPropertyDescriptor(R.prototype, "_init"), R.prototype), ge(R.prototype, "createVideo", [nt], Object.getOwnPropertyDescriptor(R.prototype, "createVideo"), R.prototype), ge(R.prototype, "initEvent", [lt], Object.getOwnPropertyDescriptor(R.prototype, "initEvent"), R.prototype), R);
function Ut(a, e) {
  var l;
  let i = a;
  i != null || (i = "");
  let t = p(/* @__PURE__ */ new Set([]));
  if (i !== "" && e.length) {
    let n = String((l = e.find((o) => o.id === i)) == null ? void 0 : l.value);
    n && (t.value = ut(n));
  }
  return {
    voiceChannel: t
  };
}
class Wt {
  constructor() {
    this.eventTarget = new EventTarget(), this.eventHandlers = /* @__PURE__ */ new Map([]);
  }
  // 监听事件
  on(e, i) {
    let t = (l) => i(l.detail);
    this.eventTarget.addEventListener(e, t), this.eventHandlers.set(e, t);
  }
  // 触发事件
  emit(e, i) {
    let t = new CustomEvent(e, {
      detail: i
    });
    this.eventTarget.dispatchEvent(t);
  }
  // 移除事件监听
  off(e) {
    let i = this.eventHandlers.get(e);
    i && (this.eventTarget.removeEventListener(e, i), this.eventHandlers.delete(e));
  }
  destroy() {
    Array.from(this.eventHandlers.keys()).forEach((i) => this.off(i));
  }
}
const z = new Wt();
function Xt() {
  return {
    isExists: (e, i) => Ee(e) !== "array" ? !1 : e.find((t) => t.sceneName === i)
  };
}
function _t(a) {
  let e = a.eventBus, i = a.video_player, t = document.querySelector(`#${a.playId}`), l = null, n = null, o = () => {
    let y = i.length;
    Array.from({
      length: y
    }, (O, Y) => {
      let I = `#${a.playId} #player_item_${Y + 1}`, K = document.querySelector(I);
      u(K, Y), d(K);
    }), c();
  }, u = (y, O) => {
    y.title = i[O].sceneName;
  }, d = (y, O) => {
    y.draggable = !0;
  }, c = () => {
    t && (t.addEventListener("click", C, {
      passive: !0
    }), t.addEventListener("dragstart", E, {
      passive: !1
    }), t.addEventListener("dragenter", f, {
      passive: !1
    }), t.addEventListener("dragleave", Q, {
      passive: !1
    }), t.addEventListener("dragend", S, {
      passive: !1
    }), t.addEventListener("dragover", M, {
      passive: !1
    }), t.addEventListener("drop", x, {
      passive: !1
    }));
  }, g = () => {
    t && (t.removeEventListener("click", C, {
      passive: !0
    }), t.removeEventListener("dragstart", E, {
      passive: !1
    }), t.removeEventListener("dragenter", f, {
      passive: !1
    }), t.removeEventListener("dragleave", Q, {
      passive: !1
    }), t.removeEventListener("dragend", S, {
      passive: !1
    }), t.removeEventListener("dragover", M, {
      passive: !1
    }), t.removeEventListener("drop", x, {
      passive: !1
    }));
  }, C = (y) => {
    if (l) {
      clearTimeout(l), l = null, e.emit("maximize", y);
      return;
    }
    l = setTimeout(() => {
      l = null, e.emit("switch", y);
    }, 200);
  }, E = (y) => {
    let O = y.target;
    O.classList.contains("maximize") || (O.classList.add("dragstart"), n = y.target);
  }, f = (y) => {
    n && n !== y.target && y.target.classList.add("dragging");
  }, Q = (y) => {
    n && n !== y.target && y.target.classList.remove("dragging");
  }, S = (y) => {
    n && (y.target.classList.remove("dragstart"), n = null);
  }, x = (y) => {
    n && (y.target.classList.remove("dragging"), e.emit("dragend", {
      target: n,
      source: y.target
    }));
  }, M = (y) => {
    n && y.preventDefault();
  };
  return {
    init: o,
    destroy: g
  };
}
const $t = "data:audio/mp4;base64,", rt = "data:image/png;base64,", At = "data:image/png;base64,", ei = "data:image/png;base64,", ti = {
  __name: "index",
  props: {
    // 模式 live vod websocket webrtc
    mode: {
      type: String,
      default: "live",
      required: !0
    },
    playStreams: {
      type: Object,
      default: () => {
      }
    },
    fileList: {
      type: Array,
      default: () => []
    },
    // mark点
    noteList: {
      type: Array,
      default: () => []
    },
    muted: {
      type: Boolean,
      default: !1
    },
    autoplay: {
      type: Boolean,
      default: !0
    },
    // 当前点播/直播的教室类型
    classroomType: {
      type: Number | String,
      default: ""
    },
    // 是否开启弹幕
    isEnableDanmu: {
      type: Boolean,
      default: !1
    },
    width: {
      type: Number,
      default: 0
    },
    height: {
      type: Number,
      default: 0
    },
    // 是启用遥控，仅在weboskcet下有效
    isEnableControl: {
      type: Boolean,
      default: !1
    },
    // 是否启用OCR，默认启用
    isEnableOcr: {
      type: Boolean,
      default: !1
      // 是否展示ocr图片转文字
    },
    // 是否启用菜单栏
    isEnableToolbar: {
      type: Boolean,
      default: !0
    },
    // 是否启用水印
    isEnableWaterMarker: {
      type: Boolean,
      default: !1
    },
    // 水印内容，开始水印时生效
    waterMarkerContent: {
      type: String,
      default: ""
    },
    // 声音通道配置
    voiceConfig: {
      type: Array,
      default: () => []
    }
  },
  emits: ["ready", "timeupdate", "startControl", "stopControl", "updateChannel"],
  setup(a, { expose: e, emit: i }) {
    const t = a;
    let { isExists: l } = Xt(), n = null, { voiceChannel: o } = Ut(t.classroomType, t.voiceConfig), u = p(!0), d = p(!1), c = p([]), g = p([]), C = p([]), E = p([]), f = p(""), Q = p(0), S = p(0), x = p(!1), M = p(/* @__PURE__ */ new Set([])), y = p(["高清", "标清"]), O = p("高清"), Y = p(["0.5", "1.0", "1.5", "2.0", "5.0"]), I = p(/* @__PURE__ */ new Set([])), K = p("1.0"), j = p(0), ht = p(!1), fe = p(localStorage.getItem("loop") === "true"), ve = p(!1), V = p(""), q = p(""), Z = p(!1), Ie = p({}), we = p(null), U = p(null), G = p(!1), P = p(!1), F = p(!1), ie = p(!1), pt = Ct(() => t.width ? {
      width: `${t.width}px`,
      height: `${t.height}px`
    } : {});
    ft(() => {
      window.addEventListener("resize", ue);
    }), vt(() => {
      ce(), window.removeEventListener("resize", ue);
    });
    let Be = () => v(this, null, function* () {
      if (u.value = !0, c.value.length) {
        let r = JSON.parse(JSON.stringify(c.value));
        r.map((s, h) => s.sceneName = Ne(s.name, h)), g.value = r;
        let A = [...g.value].slice(0, 5);
        I.value.size || (I.value = new Set(A.map((s) => s.id))), C.value = A.filter((s) => I.value.has(s.id)), yield ke(), yield new Promise((s) => setTimeout(s, 10)), It((s) => v(this, null, function* () {
          C.value.map((h, m) => {
            let D = {};
            t.mode === "vod" ? D = new jt({
              url: h.url,
              id: `#${f.value} #player_item_${m + 1}`,
              index: m,
              count: C.value.length
            }) : t.mode === "live" ? D = new Zt({
              url: h.url,
              id: `#${f.value} #player_item_${m + 1}`,
              index: m,
              count: C.value.length
            }) : t.mode === "websocket" ? D = new Yt({
              url: h.url,
              classroomId: h.id,
              name: h.sceneName,
              id: `#${f.value} #player_item_${m + 1}`,
              index: m,
              count: C.value.length,
              eventBus: z,
              isEnableControl: t.isEnableControl
            }) : t.mode === "webrtc" && (D = new Vt({
              url: h.url,
              name: h.sceneName,
              id: `#${f.value} #player_item_${m + 1}`,
              index: m,
              count: C.value.length
            })), D.id = h.id, D.sceneName = h.sceneName, E.value.push(D);
          }), E.value.map((h) => {
            le(h), h.onLoadFinish = () => {
              console.log("load finish"), le(h), d.value || (Qe(h), u.value = !1), d.value = !0;
            };
          }), yield De(), yield new Promise((h) => setTimeout(h, 5e3)), u.value = !1, t.mode === "vod" && !d.value && E.value.length && N();
        }));
      } else
        u.value = !1;
    }), Qe = (r) => v(this, null, function* () {
      if (yield Me(r), yield k("muted"), Q.value && t.mode === "vod" && (yield k("current", Q.value)), t.autoplay || ie.value ? yield k("play") : yield k("pause"), t.mode === "vod" && (yield ne()), t.autoplay)
        try {
          yield je(), console.log("Allow play with voice");
        } catch (A) {
          console.log("Not allow play with voice"), yield k("volume", 0);
        }
      !t.muted && (yield be()), t.isEnableToolbar && (yield ae()), ie.value = !0, i("ready");
    }), De = () => {
      E.value.length, n = _t({ eventBus: z, video_player: E.value, playId: f.value }), n.init(), z.on("switch", (r) => {
        T("switch", r);
      }), z.on("maximize", (r) => {
        T("maximize", r);
      }), z.on("dragend", (r) => {
        Ae(r.target, r.source);
      });
    }, k = (r, A) => {
      E.value.map((s, h) => {
        switch (r) {
          case "play":
            s.isPlaying() || s.play();
            break;
          case "pause":
            s.pause();
            break;
          case "replay":
            s.seek(0), s.play();
            break;
          case "destroy":
            s.release();
            break;
          case "muted":
            M.value.has(s.sceneName) ? s.setVoice(+(j.value / 100)) : s.setVoice(0);
            break;
          case "rate":
            s.setPlaybackRate(+A), K.value !== A && (K.value = A);
            break;
          case "current":
            s.seek(A), Q.value = A;
            break;
          case "volume":
            M.value.has(s.sceneName) && s.setVoice(+(A / 100)), j.value = +A;
            break;
        }
      });
    }, Me = (r) => {
      r.onPlayTime = (A) => {
        A && (Q.value = A), t.mode === "vod" && i("timeupdate", A);
      }, r.onPlayState = (A) => {
        x.value = A;
      }, r.onPlayFinish = (A) => {
        x.value = !1, fe.value && k("replay");
      }, t.mode === "websocket" && (z.on("startControl", (A) => {
        i("startControl", A);
      }), z.on("stopControl", (A) => {
        i("stopControl", A);
      }));
    }, T = (r, A) => {
      ye((h, m) => {
        switch (h) {
          case "play":
            Re();
            break;
          case "definition":
            Se(m);
            break;
          case "muted":
            xe(m);
            break;
          case "loop":
          case "debounce":
          case "optimization":
            localStorage.setItem(h, m);
            break;
          case "fullscreen":
            Oe();
            break;
          case "flip":
            break;
          case "channel":
            Le(m);
            break;
          case "ocr":
            ze(m);
            break;
          case "maximize":
            re(m), P.value && C.value.length === 2 ? Ke(m) : He(m);
            break;
          case "screening":
            m === "movie" ? qe() : m === "pip" ? Pe() : m === "multiSplitScreen" && Fe();
            break;
          case "switch":
            let D = C.value.length;
            if (m.target.classList.contains("maximize"))
              return;
            let H = document.querySelector(`#${f.value} #player_item_${D}`);
            D === 2 && m.target.id === "player_item_2" && (H = document.querySelector(`#${f.value} #player_item_1`)), Ae(m.target, H);
            break;
          default:
            throw new Error("Can not find！");
        }
      }, 10, r, A);
    }, N = (r = 0) => v(this, null, function* () {
      yield oe(), yield new Promise((A) => setTimeout(A, r)), yield Be();
    }), ke = (r) => {
      f.value = `player_list_${Date.now()}-${Math.random().toString(36).substring(2)}`;
    }, be = (r) => {
      if (E.value.length) {
        let A = [];
        A = E.value.filter((s) => o.value.has(s.sceneName)), !A.length && (A = [E.value.find((s) => s.sceneName === "老师") || E.value.find((s) => s.sceneName === "老师全景") || E.value[E.value.length - 1]]), A.map((s) => T("muted", s));
      }
    }, ae = (r) => {
      t.isEnableWaterMarker && (q.value = `player_wrap_${(/* @__PURE__ */ new Date()).getTime()}`, V.value = new Nt({ id: q.value, content: t.waterMarkerContent }));
    }, Re = () => {
      x.value = !x.value, k(x.value ? "play" : "pause");
    }, ne = () => v(this, null, function* () {
      E.value.map((r) => {
        var s;
        let A = ((s = r.mediaInfo()) == null ? void 0 : s.duration) / 1e3;
        (!S.value || Number(A) > S.value) && (S.value = A);
      }), S.value || (yield new Promise((r) => setTimeout(r, 500)), ne());
    }), Se = (r) => {
      O.value = r, c.value = t.playStreams[O.value].source || [], N(20);
    }, xe = (r) => {
      M.value.has(r.sceneName) ? M.value.delete(r.sceneName) : M.value.add(r.sceneName), M.value = new Set(M.value), k("muted");
    }, Oe = () => {
      we.value.handleChange(document.getElementById("video_player")), setTimeout((r) => {
        t.mode === "vod" && U.value._getDistance();
      }, 40);
    }, Le = (r) => {
      I.value.has(r) ? I.value.delete(r) : I.value.add(r), I.value = new Set(I.value), Ge(), N(20);
    }, Ge = (r) => {
      I.value.size > 1 && G.value && (G.value = !1);
    }, ze = (r) => {
      let A = E.value.find((s) => s.id === r.id);
      if (t.mode !== "vod") {
        re({ target: "" });
        let s = Te(r.id);
        s && !s.classList.contains("maximize") && T("maximize", { target: s });
      }
      "startViewTransition" in document ? document.startViewTransition(() => {
        Z.value = !0;
      }).ready.then(() => {
        document.querySelector("#ocrDialog").animate(
          [
            {
              top: "60%",
              opacity: 0.5
            },
            {
              top: "50%",
              opacity: 1
            }
          ],
          {
            duration: 200,
            easing: "cubic-bezier(0.42, 0, 0.58, 1)",
            fill: "forwards"
            // 动画结束后保持最后一帧状态
          }
        );
      }) : Z.value = !0, setTimeout((s) => {
        var h;
        Ie.value = {
          item: se(A),
          id: (h = se(A)) == null ? void 0 : h.id
        };
      }, 100);
    }, gt = (r) => {
      Z.value = !1;
    }, le = (r) => {
      if (r.player)
        try {
          t.mode === "live" ? (r.player.children_[0].id = `video_${r.id}`, r.player.children_[0].classList = ["video__"]) : t.mode === "vod" ? (r.player.id = `video_${r.id}`, r.player.classList = ["video__"]) : t.mode === "websocket" ? (r.player.player.id = `video_${r.id}`, r.player.player.classList = ["video__"]) : t.mode;
        } catch (A) {
        }
    }, Ke = (r) => {
      let A = document.querySelector(".two_split_screen"), [s, h] = A.children, { target: m } = r;
      if ([s, h].includes(m)) {
        let D = m === s ? h : s, H = m.classList.contains("pip_sub");
        m.classList.toggle("pip_sub", !H), m.classList.toggle("pip_main", H), D.classList.toggle("pip_sub", H), D.classList.toggle("pip_main", !H);
      }
    }, He = (r) => {
      r.target.classList.contains("maximize") ? (r.target.classList.remove("maximize"), r.target.setAttribute("draggable", !0)) : (r.target.classList.add("maximize"), r.target.removeAttribute("draggable"));
    }, re = (r) => {
      document.querySelectorAll(".maximize").forEach((A) => A !== r.target ? A.classList.remove("maximize") : "");
    }, qe = () => {
      if (G.value = !G.value, G.value) {
        let r = l(C.value, "PGM");
        I.value = /* @__PURE__ */ new Set([r.id]);
      } else
        I.value = /* @__PURE__ */ new Set([]);
      N(20);
    }, Pe = () => {
      P.value = !P.value;
    }, Fe = () => {
      P.value && (P.value = !1, C.value.length === 2) || (G.value && (G.value = !1), I.value = /* @__PURE__ */ new Set([]), N(20));
    }, Ae = (r, A) => {
      if (r === A)
        return;
      A.insertBefore(r.firstChild, A.firstChild), r.appendChild(A.childNodes[1]);
      let s = r.getAttribute("title"), h = A.getAttribute("title");
      r.setAttribute("title", h), A.setAttribute("title", s);
    }, Te = (r) => {
      var A, s;
      return t.mode === "live" ? (A = document.getElementById(`video_${r}`)) == null ? void 0 : A.parentElement.parentElement : (s = document.getElementById(`video_${r}`)) == null ? void 0 : s.parentElement;
    }, se = (r) => {
      switch (t.mode) {
        case "vod":
          return r.player;
        case "live":
          return r.player.children_[0];
        case "websocket":
          return r.player.player;
        case "webrtc":
          return r.player;
      }
    }, oe = () => {
      n == null || n.destroy(), z.destroy(), k("destroy"), q.value && (V.value.destroy(), q.value = ""), d.value = !1, E.value = [], M.value = /* @__PURE__ */ new Set([]);
    }, ce = () => {
      oe(), C.value = [], I.value = /* @__PURE__ */ new Set([]), ve.value = !1, K.value = "1.0", Q.value = 0, S.value = 0, j.value = 70, x.value = !1;
    }, mt = (r) => {
      (r.key === "Escape" || r.key === "F11") && setTimeout((A) => {
        t.mode === "vod" && U.value._getDistance();
      }, 40);
    }, ue = () => {
      ye((r) => {
        parent.document.webkitIsFullScreen || parent.document.fullscreen || parent.document.mozFullScreen || parent.document.msFullscreenElement, t.mode === "vod" && U.value._getDistance(), q.value && (V.value.destroy(), ae());
      }, 200);
    }, Ne = (r, A) => (r || (r = `通道${A + 1}`), r.replace("教师", "老师").toUpperCase()), je = (r) => v(this, null, function* () {
      return new Promise((A, s) => {
        let h = new Audio();
        h.src = $t, h.volume = 0.01, h.play().then(() => {
          A(), F.value = !1;
        }).catch(() => {
          s(), F.value = !0;
        });
      });
    }), yt = (r) => v(this, null, function* () {
      F.value = !1, k("volume", 70);
    });
    return de(() => t.playStreams, (r) => {
      if (Ee(r === "object") && Object.keys(r).length) {
        ce(), y.value = Object.keys(r);
        for (let A of y.value) {
          let s = r[A].source;
          if (s && s.length) {
            T("definition", A);
            break;
          }
        }
      }
    }, {
      deep: !0,
      immediate: !0
    }), de(() => I.value, (r) => {
      i("updateChannel", r.size);
    }), de(() => j.value, (r) => {
      r && F.value && (F.value = !1);
    }), e({
      _register_emits: k,
      current: Q,
      duration: S
    }), { __sfc: !0, isExists: l, use_player_emit: n, props: t, emits: i, voiceChannel: o, is_loading: u, is_init: d, fileList: c, fileListCustom: g, resource_list: C, video_player: E, playId: f, current: Q, duration: S, play: x, voice: M, definition_list: y, definition: O, rate_list: Y, channel_list: I, rate: K, volume: j, fullscreen_status: ht, loop: fe, flip: ve, waterMarker: V, waterMarkerId: q, show_ocr: Z, ocr_origin: Ie, fullscreen: we, slide: U, isMovieMode: G, isPipMode: P, isShowVolumeTip: F, isDisabledAutoplay: ie, videoPlayerStyle: pt, __init: Be, initializePlayer: Qe, _registerDomEvent: De, _register_emits: k, _listener_emit: Me, handle_click: T, _replyRenderPlayer: N, _setPlayId: ke, _handleDefaultVoice: be, _handleWaterMarker: ae, _play: Re, _get_duration: ne, _handleChangeDefinition: Se, _handleChangeMutedChannel: xe, _handleRequestFullscreen: Oe, _handleChangeChoiceChannel: Le, _handleCancelMovieMode: Ge, _handleShowOcrModel: ze, cancelOcrModel: gt, _updateVideoStyle: le, _handleClickPip: Ke, _handlerMaximization: He, _handleCancelAllMaximization: re, _handleChangeScreeningToMovie: qe, _handleChangeScreeningToPip: Pe, _handleChangeScreeningToMultiSplit: Fe, _handleSwitchVideo: Ae, _getPlayerItemWrap: Te, _getVideoDom: se, _dispose: oe, _reset: ce, _listen_keyup_emits: mt, _listener_fullscreen_status: ue, _filterChannelName: Ne, _isAllowPlayByVoice: je, handleOpenVoiceByClick: yt, Slide: Kt, common: dt, FullScreen: Tt };
  }
};
var ii = function() {
  var e = this, i = e._self._c, t = e._self._setupProxy;
  return i("div", { staticClass: "video_player_pc _flex_center", style: t.videoPlayerStyle, attrs: { id: "video_player" } }, [i("div", { staticClass: "player_wrap", class: { player_wrap_pip: t.isPipMode && t.resource_list.length === 2 }, style: t.resource_list.length === 1 ? { width: "auto", height: "100%" } : {}, attrs: { id: `${t.waterMarkerId}` } }, [t.resource_list.length === 1 ? i("div", { staticClass: "player_list default_screen", attrs: { id: `${t.playId}` } }, [i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, attrs: { id: "player_item_1" } })]) : e._e(), t.resource_list.length === 2 ? i("div", { staticClass: "player_list two_split_screen _flex_item_center", attrs: { id: `${t.playId}` } }, [i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip, pip_main: t.isPipMode }, staticStyle: { width: "50%" }, attrs: { id: "player_item_1" } }), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip, pip_sub: t.isPipMode }, staticStyle: { width: "50%" }, attrs: { id: "player_item_2" } })]) : e._e(), t.resource_list.length === 3 ? i("div", { staticClass: "player_list _flex_item_center three_split_screen", attrs: { id: `${t.playId}` } }, [i("div", { staticStyle: { width: "50%", height: "100%" } }, [i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, staticStyle: { height: "50%" }, attrs: { id: "player_item_1" } }), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, staticStyle: { height: "50%" }, attrs: { id: "player_item_2" } })]), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, staticStyle: { width: "100%" }, attrs: { id: "player_item_3" } })]) : e._e(), t.resource_list.length === 4 ? i("div", { staticClass: "player_list _flex_item_center four_split_screen", attrs: { id: `${t.playId}` } }, [i("div", { staticStyle: { width: "26.6%" } }, [i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, attrs: { id: "player_item_1" } }), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, attrs: { id: "player_item_2" } }), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, attrs: { id: "player_item_3" } })]), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, attrs: { id: "player_item_4" } })]) : e._e(), t.resource_list.length === 5 ? i("div", { staticClass: "player_list _flex_item_center five_split_screen", attrs: { id: `${t.playId}` } }, [i("div", { staticStyle: { width: "25%" } }, [i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, attrs: { id: "player_item_1" } }), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, attrs: { id: "player_item_2" } }), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, attrs: { id: "player_item_3" } }), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, attrs: { id: "player_item_4" } })]), i("div", { staticClass: "player_item _flex_item_center", class: { flip: t.flip }, staticStyle: { width: "100%" }, attrs: { id: "player_item_5" } })]) : e._e(), t.is_init ? e._e() : i("div", { staticClass: "player_loading _flex_center" }, [i("img", { attrs: { src: te, alt: "" } })])]), e.isEnableToolbar ? i("footer", { style: t.is_loading ? { pointerEvents: "none" } : {} }, [e.mode === "vod" ? i(t.Slide, { ref: "slide", attrs: { duration: +t.duration, mark: e.noteList }, on: { update: (l) => t._register_emits("current", l) }, model: { value: t.current, callback: function(l) {
    t.current = l;
  }, expression: "current" } }) : e._e(), i("div", { staticClass: "toolbar _flex_item_center" }, [i("div", { staticClass: "playing _flex_center", on: { click: function(l) {
    return t.handle_click("play");
  } } }, [i("i", { class: `iconfont ${t.play ? "icon-24gf-pause2" : "icon-24gf-play"}` })]), i("div", { staticClass: "time" }, [e._v(" " + e._s(t.common.filterTime(t.current, 1)) + " "), e.mode === "vod" ? i("span", { staticStyle: { "font-weight": "bold" } }, [e._v("  /  " + e._s(t.common.filterTime(t.duration, 1)) + " ")]) : e._e()]), i("el-popover", { staticStyle: { "margin-left": "auto" }, attrs: { placement: "top", "append-to-body": !1, "popper-options": { boundariesElement: ".toolbar", removeOnDestroy: !1 }, width: "50px", "popper-class": "popper_custom", trigger: "hover" } }, [i("div", { staticClass: "select_list mode_list" }, [i("div", { staticClass: "item _flex_center", class: { active: t.isMovieMode }, attrs: { disabled: !t.isExists(t.resource_list, "PGM") }, on: { click: function(l) {
    return t.handle_click("screening", "movie");
  } } }, [i("i", { staticClass: "iconfont icon--" }), i("span", [e._v("电影模式")])]), i("div", { staticClass: "item _flex_center", class: { active: t.isPipMode }, attrs: { disabled: t.resource_list.length !== 2 }, on: { click: function(l) {
    return t.handle_click("screening", "pip");
  } } }, [i("i", { staticClass: "iconfont icon-btn_trophy_defaultbeifen" }), i("span", [e._v("画中画")])]), i("div", { staticClass: "item _flex_center", class: { active: !t.isMovieMode && !t.isPipMode }, on: { click: function(l) {
    return t.handle_click("screening", "multiSplitScreen");
  } } }, [i("i", { staticClass: "iconfont icon-erjicaidan--duopingmu" }), i("span", [e._v("多分屏")])])]), i("div", { staticClass: "definition mode _flex_center", attrs: { slot: "reference" }, slot: "reference" }, [i("span", [e._v(e._s(t.isMovieMode ? "电影模式" : t.isPipMode ? "画中画" : "多分屏"))])])]), e.isEnableOcr ? i("el-popover", { attrs: { placement: "top", width: "50px", "append-to-body": !1, "popper-options": { boundariesElement: ".toolbar", removeOnDestroy: !1 }, "popper-class": "popper_custom", trigger: "hover" } }, [i("div", { staticClass: "select_list ocr_list" }, e._l(t.resource_list, function(l, n) {
    return i("div", { key: n, staticClass: "item _flex_center", attrs: { disabled: e.muted }, on: { click: function(o) {
      return t.handle_click("ocr", l);
    } } }, [i("i", { staticClass: "iconfont icon-ocr" }), i("span", [e._v(e._s(l.sceneName))])]);
  }), 0), i("div", { staticClass: "definition _flex_center", attrs: { slot: "reference" }, slot: "reference" }, [i("span", [e._v("OCR")])])]) : e._e(), i("el-popover", { attrs: { placement: "top", width: "50px", "append-to-body": !1, "popper-options": { boundariesElement: ".toolbar", removeOnDestroy: !1 }, "popper-class": "popper_custom", trigger: "hover" } }, [i("div", { staticClass: "select_list" }, e._l(t.definition_list, function(l) {
    return i("div", { staticClass: "item _flex_center", class: { active: t.definition === l }, on: { click: function(n) {
      return t.handle_click("definition", l);
    } } }, [i("span", [e._v(e._s(l))])]);
  }), 0), i("div", { staticClass: "definition _flex_center", attrs: { slot: "reference" }, slot: "reference" }, [i("span", [e._v(e._s(t.definition))])])]), e.mode === "vod" ? i("el-popover", { attrs: { placement: "top", width: "50px", "append-to-body": !1, "popper-options": { boundariesElement: ".toolbar", removeOnDestroy: !1 }, "popper-class": "popper_custom popper_custom_rate", trigger: "hover" } }, [i("div", { staticClass: "select_list" }, e._l(t.rate_list, function(l, n) {
    return i("div", { key: n, staticClass: "item _flex_center", class: { active: t.rate === l }, on: { click: function(o) {
      return t._register_emits("rate", l);
    } } }, [i("span", [e._v(e._s(l) + "x")])]);
  }), 0), i("div", { staticClass: "rate _flex_center", attrs: { slot: "reference" }, slot: "reference" }, [i("span", [e._v(e._s(t.rate) + "x")])])]) : e._e(), i("el-popover", { attrs: { placement: "top", width: "50px", "append-to-body": !1, "popper-options": { boundariesElement: ".toolbar", removeOnDestroy: !1 }, "popper-class": "popper_custom popper_custom_voice", trigger: "hover" } }, [i("div", { staticClass: "select_list channel_list" }, e._l(t.fileListCustom, function(l, n) {
    return i("div", { key: n, staticClass: "item _flex_center", class: { active: t.channel_list.has(l.id) }, on: { click: function(o) {
      return t.handle_click("channel", l.id);
    } } }, [i("i", { class: `iconfont ${t.channel_list.has(l.id) ? "icon-xianshikejian" : "icon-yincangbukejian"}` }), i("span", [e._v(e._s(l.sceneName))])]);
  }), 0), i("div", { staticClass: "channel _flex_center", attrs: { slot: "reference" }, slot: "reference" }, [i("i", { staticClass: "iconfont icon-tuwenliebiao" })])]), i("el-popover", { attrs: { placement: "top", width: "80px", "append-to-body": !1, "popper-options": { boundariesElement: ".toolbar", removeOnDestroy: !1 }, "popper-class": "popper_custom popper_custom_voice", trigger: "hover" } }, [i("div", { staticClass: "select_list voice_list" }, e._l(t.resource_list, function(l, n) {
    return i("div", { key: n, staticClass: "item _flex_center", class: { active: t.voice.has(l.sceneName) }, attrs: { disabled: e.muted }, on: { click: function(o) {
      return t.handle_click("muted", l);
    } } }, [t.voice.has(l.sceneName) ? i("img", { attrs: { src: rt, alt: "" } }) : i("img", { attrs: { src: At, alt: "" } }), i("span", [e._v(e._s(l.sceneName))])]);
  }), 0), i("div", { staticClass: "channel _flex_center", attrs: { slot: "reference" }, slot: "reference" }, [i("i", { staticClass: "iconfont icon-yinpinliebiao" })])]), i("el-popover", { attrs: { placement: "top", width: "160px", "append-to-body": !1, "popper-options": { boundariesElement: ".toolbar", removeOnDestroy: !1 }, trigger: "manual", "popper-class": "popper_custom popper_custom_voice" }, model: { value: t.isShowVolumeTip, callback: function(l) {
    t.isShowVolumeTip = l;
  }, expression: "isShowVolumeTip" } }, [i("div", { staticStyle: { padding: "0.03rem" } }, [i("span", { staticStyle: { color: "white" } }, [e._v("检测到声音未开启！")]), i("br"), i("el-button", { staticStyle: { "margin-top": "0.07rem" }, attrs: { type: "primary", size: "mini" }, on: { click: t.handleOpenVoiceByClick } }, [e._v("点击开启")])], 1), i("div", { staticClass: "voice _flex_item_center", attrs: { slot: "reference" }, slot: "reference" }, [i("div", { staticClass: "voice_btn _flex_center", on: { click: function(l) {
    return t._register_emits("volume", t.volume ? 0 : 70);
  } } }, [t.volume === 0 ? i("img", { attrs: { src: At, alt: "" } }) : t.volume >= 50 ? i("img", { attrs: { src: rt, alt: "" } }) : t.volume > 0 ? i("img", { attrs: { src: ei, alt: "" } }) : e._e()]), i("div", { staticClass: "voice_slide" }, [i("el-slider", { staticStyle: { width: "100%" }, attrs: { min: 0, max: 100 }, on: { change: function(l) {
    return t._register_emits("volume", t.volume);
  } }, model: { value: t.volume, callback: function(l) {
    t.volume = l;
  }, expression: "volume" } })], 1)])]), i(t.FullScreen, { ref: "fullscreen", attrs: { type: 1 }, model: { value: t.fullscreen_status, callback: function(l) {
    t.fullscreen_status = l;
  }, expression: "fullscreen_status" } }, [i("div", { staticClass: "full _flex_center", on: { click: function(l) {
    return t.handle_click("fullscreen");
  } } }, [t.fullscreen_status ? i("i", { staticClass: "iconfont icon-quxiaoquanping" }) : i("i", { staticClass: "iconfont icon-quanping1" })])])], 1)], 1) : e._e(), e._t("ocr", null, { show_ocr: t.show_ocr, mode: e.mode, fullscreen: t.fullscreen_status, origin: t.ocr_origin, curPlayTime: t.current, cancel: t.cancelOcrModel })], 2);
}, ai = [], ni = /* @__PURE__ */ Ce(
  ti,
  ii,
  ai,
  !1,
  null,
  "9fe8fcfa"
);
const li = ni.exports, hi = (a, e) => {
  a.component("VueMultiSplitPlayer", li);
};
export {
  li as VueMultiSplitPlayer,
  hi as install
};
//# sourceMappingURL=vue-multi-split-player.es.js.map
